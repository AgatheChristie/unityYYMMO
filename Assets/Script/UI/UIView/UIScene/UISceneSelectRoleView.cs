...
public class UISceneSelectRoleView : UISceneViewBase
{
    ...
    {
        GameObjectUtil.QSetTransformActive(UICreateRole, isActive);
    ...
    {
        GameObjectUtil.QSetTransformActive(UISelectRole, isActive);
    ...
    public void SetRoleList(List<Role_Get_Role_List_S2CProto.RoleItem> list, Action<int> OnSelectRole)
    {
        ClearRoleListUI();
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            if (view != null)
            {
                view.SetUI(list[i].RoleId, list[i].RoleNickName, list[i].RoleLevel, list[i].RoleCareer, m_RoleHeadPic[list[i].RoleCareer - 1], OnSelectRole);
